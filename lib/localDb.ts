 ...
 * e o cliente saiba que precisa retentar.
 */
...
/**
 * Em produção, leituras que caíram no fallback localDb indicam que o
 * Prisma está fora — o resultado será stale ou vazio. Não bloqueamos
 * (pode mascarar erros piores se a UI estourar com null), mas logamos
 * pra que o problema apareça em Sentry/logs do Railway.
 */
export function warnIfFallbackReadInProd(context: string) {
  if (isProductionWithDb) {
    console.error(
      `[localDb] FALLBACK READ EM PRODUÇÃO: ${context}. Prisma indisponível, retornando dados do JSON local (provavelmente vazio).`,
    )
  }
}
